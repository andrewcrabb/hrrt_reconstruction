...
void allocation_memory() {
  ...
  else if (verbose & 0x0040) 
    LOG_INFO("Allocate imageprj3: {} kbytes",     views / 2 * segmentsize * 2 * xr_pixels / 256);
...
void GetScannerParameter()
{
  ...
  if(iModel!=0){
    LOG_INFO("Model %d", iModel);
    model_info    = scanner_model(iModel);     /* assumed code for HRRT */
  } else {
    LOG_INFO("Model %s", imodeldefname); 
    model_info=(ScannerModel *) malloc(sizeof(ScannerModel));
    tmpfp=fopen(imodeldefname,"rt");
    if (tmpfp == NULL)
      LOG_EXIT("Open {}, iModel=0, please try again with right scanner definition file",imodeldefname);
...
void Output_CorrectedScan()
{
  ...
  if ((tmpfp=fopen(in_img_file,"rb"))==NULL) 
        LOG_EXIT("  Main(): error in read_flat_image() when reading the initial image from %s", in_img_file );
...
int main(int argc, char* argv[]) {
  ...
  if (ssinofp != (FILEPTR)0) {
    ...
    if (fstat(fileno(ssinofp), &st) == 0) {
      ...
      LOG_EXIT("%s: Error getting file size\n", scat_scan_file);
  }
