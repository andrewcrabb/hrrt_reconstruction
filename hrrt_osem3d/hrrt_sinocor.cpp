...
void allocation_memory() {
	...
	if( imageprj3 == NULL ) 
		LOG_EXIT ("  Error in allocation memory for imageprj3");
	else 
		LOG_DEBUG("  Allocate imageprj3  : {} kbytes", 
  ...
  if (chFlag)
  {
    ...
    if( delayedprj3 == NULL ) 
    	LOG_EXIT ("Error in allocation memory for delayedprj3");
    else 
    	LOG_DEBUG("Allocate delayedprj3  : {} kbytes",
      views/2*segmentsize*2*xr_pixels/256);
  }
...
//		imagexyzf_thread[i]=matrix3dm128(0,x_pixels-1,0,y_pixels-1,0,z_pixels_simd*4);
	if( imagexyzf_thread[i] == NULL ) 
		LOG_EXIT("allocating thread recon buffer");
...
//		prjxyf_thread[i]=(float ***) matrix3dm128(0,xr_pixels,0,th_pixels/2,0,yr_pixels);
		if( prjxyf_thread[i] == NULL ) 
			LOG_EXIT("allocating thread recon buffer2");
	}
}
...
void alloc_imagexyzf()
{
	...
	if( imagexyzf == NULL ) 
		LOG_EXIT("emory allocation failure for imagexyzf");
	else 
...
void alloc_prjxyf()
{
	...
	if( prjxyf == NULL ) 
		LOG_EXIT("emory allocation failure for prjxyf");
	else 
...
void CheckFileValidate()
{
  if (true_file!=NULL) {            
		...
		if(tsinofp == FOPEN_ERROR) LOG_EXIT(" cannot open input true sinogram file {}",true_file);
	} 
	else {            
		psinofp = file_open(prompt_file,"rb");
		if(psinofp == FOPEN_ERROR) LOG_ERROR("cannot open input prompt sinogram file '{}'",prompt_file);
		if(psinofp == FOPEN_ERROR) LOG_EXIT(" cannot open input prompt sinogram file {}",prompt_file);
		dsinofp = file_open(delayed_file,"rb");
		if(dsinofp == FOPEN_ERROR) LOG_ERROR("cannot open input delayed sinogram file {}",delayed_file);
		if(dsinofp == FOPEN_ERROR) LOG_EXIT(" cannot open input delayed sinogram file {}",delayed_file);
	}

  normfp = file_open(norm_file,"rb");
  if(normfp == FOPEN_ERROR) LOG_ERROR("cannot open normalization file {}",norm_file);
  if(normfp == FOPEN_ERROR) LOG_EXIT(" cannot open normalization file {}",norm_file);

  if (atten_file != NULL) {            
		attenfp = file_open(atten_file,"rb");
		if(attenfp == FOPEN_ERROR) LOG_ERROR("cannot open attenuation file {}",atten_file);    /* attenfp could be null */
		if(attenfp == FOPEN_ERROR) LOG_EXIT(" cannot open attenuation file {}",atten_file);    /* attenfp could be null */
	}

	if(scat_scan_file!=NULL) {            
		ssinofp = file_open(scat_scan_file,"rb");
		if(ssinofp == FOPEN_ERROR) LOG_ERROR("cannot open input scatter sinogram file {}",scat_scan_file);
		if(ssinofp == FOPEN_ERROR) LOG_EXIT(" cannot open input scatter sinogram file {}",scat_scan_file);
	}

  /* check initial image file existence */
  if (access(in_img_file,R_OK) != 0)
    LOG_EXIT("error initial image file {} not found", in_img_file );
}
...
void Output_CorrectedScan()
{
  ...
  if ((stmp=(short*)calloc(npixels, sizeof(short))) == NULL ||
    ...
    LOG_EXIT("memory allocation failure for true 2D sinogram");
  if (tsinofp == NULL)
  {
    if ((ra=(float*)calloc(npixels, sizeof(float))) == NULL)
      LOG_EXIT("memory allocation failure for smoothed randoms 2D sinogram");
  }
  if ((no=(float*)calloc(npixels, sizeof(float))) == NULL)
    LOG_EXIT("memory allocation failure for norm 2D sinogram");
  if (attenfp != NULL)
  {
    if ((at=(float*)calloc(npixels, sizeof(float))) == NULL)
    LOG_EXIT("memory allocation failure for atten 2D sinogram");
  }
  if (ssinofp != NULL)
  {
    if ((sc=(float*)calloc(npixels, sizeof(float))) == NULL)
    LOG_EXIT("memory allocation failure for scatter 2D sinogram");
  }
  if (s2DFlag) { // pre-load unscaled scatter and scale factors
    ...
    if (sc2d==NULL || scale_factors==NULL) 
      LOG_EXIT("utput_CorrectedScan: memory allocation error");
    if( fread(sc2d, sizeof(float), npixels*z_pixels, ssinofp) != 
      npixels*z_pixels)  
      LOG_EXIT("utput_CorrectedScan: error reading unscaled 2D scatter!");
    if (fread(scale_factors, sizeof(float), segmentsize, ssinofp) != segmentsize)
      LOG_EXIT("repare_osem3dw0: error reading scatter scale factors!");
  }

  image = (float ***) matrix3dm128(0,x_pixels-1, 0,y_pixels-1, 0,z_pixels_simd-1);  
  if( image == NULL ) LOG_EXIT("matrix3dm128 error! memory allocation failure for image");
  else 
  	LOG_DEBUG("  Allocate initial image : %d kbytes", imagesize/256);
  if ((tmpfp=fopen(in_img_file,"rb"))==NULL) 
				LOG_EXIT("error in read_flat_image() when reading the initial image from {}", in_img_file );
  if ((ptr2d=matrixfloat(0,x_pixels-1,0,y_pixels-1)) == NULL)
    LOG_EXIT("matrixfloat error! memory allocation failure for image");
  ...
  for(v=0;v<views/2;v++)
  {
    ...
    if( !forward_proj3d2(image, estimate, v,nthreads,imagexyzf,prjxyf))
      LOG_EXIT("error occurs in forward_proj3d!");
    for(yr2=0;yr2<segmentsize*2;yr2++) 
      memcpy(imageprj[v][yr2],estimate[yr2],xr_pixels*sizeof(float));
  }

	time(&t2);
	LOG_INFO("  Osem3d forward projection done in {} sec", t2-t1);
	...
	for(theta=0;theta<th_pixels;theta++)
  {
		...
		for( yr=yr_bottom[theta]; yr<=yr_top[theta]; yr++, yr2++, plane++)
    {
      ...
      if (fwrite(emc, npixels*sizeof(float), 1, tmpfp) != 1 )
         LOG_EXIT("orrected sinogram write error");
        
       LOG_INFO("   plane {} done", yr2);
     }  // end yr
   }  // end theta
	fclose(tmpfp);
...
//	InitParameter();
	...
	if( !dependencies(nprojs, nviews, verbose) ) LOG_EXIT("Error occur in dependencies()");
  if (ssinofp != (FILEPTR)0) {
    ...
    if (stat(osem3dpar->scat_scan_file, &st) == 0) {
      if (st.st_size == (xr_pixels*views*z_pixels + segmentsize_9)*sizeof(float)) {
    	...
    	LOG_EXIT("{}: Error getting file size\n", scat_scan_file);
  }
