...
static void write_header(char **in_data_fnames, int nfiles, char *out_data_fname)
{
	...
	if ((out_fp=fopen(out_hdr_fname,"wb")) == NULL) perror(out_hdr_fname);
...
static void compute_norm(char **in_files, int nfiles, char *out_file)
{
	...
	if (data_size== (size_t)(nelems*nviews*ntotal_3*sizeof(short))) span = 3;
	else if (data_size == (size_t)(nelems*nviews*ntotal_9*sizeof(short))) span = 9;
	else 
	{
      printf("%s size= %d : not span3 or span9 size\n", in_files[0], (int)data_size);
		exit(1);
	}
	for (file_num=1; file_num<nfiles; file_num++)
	{
		...
		{
			perror(in_files[file_num]);
			exit(1);
		}
		if (data_size != (size_t)st.st_size) 
		{
			printf("Error: input files should have the same size\n");
			exit(1);
		}
		printf("%s: size OK\n", in_files[file_num]);
	}
...
//	r1 = indgen(nrings*nrings) mod nrings	;  
//	r2 = (indgen(nrings*nrings)-r1)/nrings
	...
	try
	{
		...
		if ((sino = (short*)calloc(nvoxels, sizeof(short))) == NULL)throw(malloc_error);
		if (nfiles>1) {
			if ((tmp_sino = (short*)calloc(nvoxels, sizeof(short))) == NULL)throw(malloc_error);
		}
		if ((norm = (float*)calloc(nvoxels, sizeof(float))) == NULL)throw(malloc_error);
...
//		why higherf threshold? span norm 9 values are lower than span 3 
		...
		if (qc_flag) printf ("keep only normalization between %f and %f\n",norm_min,norm_max);
		//
		// We will need to correct for the trajectory of the rod, 
		// so fit it an save parameters in array "a"
		//
		// IDL: sinogram = float(st(0))
		// IDL: a=fit_dwell(sinogram)
		if (fread(sino, npixels*sizeof(short), nplns, in_fp[0]) != (size_t)nplns) throw(read_error);
		for (file_num=1; file_num<nfiles; file_num++)
		{
		...
		printf("Fitting dwell ...");
		if ((a_array = fit_dwell(sino, nplns0)) == NULL) throw (fit_error);
		...
		while (iseg<nseg)
		{
			...
			if (seg != 0 ) 
			{
				...
				//IDL:	st = assoc(ilun,intarr(nelems,nviews,nplns),2*offset)
				//IDL:	sinogram = float(st(0)) > 0.0
				if (fread(sino, npixels*sizeof(short), nplns, in_fp[0]) != (size_t)nplns) throw(read_error);
				for (file_num=1; file_num<nfiles; file_num++)
				{
					if (fread(tmp_sino, npixels*sizeof(short), nplns, in_fp[file_num]) == (size_t)nplns) 
			...
			printf("Segment %3d @ phi %6.4f deg \t NumPlanes: %d \t Offset: %d\n",
                   seg * sign, phi_r * 180 / M_PI, nplns, (int)offset);
			...
			for (int view = 0; view<nviews; view++, angle += d_angle)
			{
				...
				//
				// IDL: dwell = sqrt((a(1,z)^2-(x-a(2,z)*cos(angle-a(3,z)))^2) > 0.)
				// IDL: dwell = dwell > .01*max(dwell) 
				// IDL: dwell = a(0,z)/dwell
				// IDL: dwell = reform(dwell,nelems,nplns)
				...
				for (pl=0; pl<nplns; pl++)
				{
					...
					for (j=0; j<nelems; j++, x++)
					{
						...
						if ((dwell = a1_sq - (v2*v2)) > 0.0f)
						{
							...
							if (dwell>dwell_max) dwell_max = dwell;
						}
						else dwell = 0.0f;
						*dwellp++ = dwell;
					}	
				...
				if (seg==0 && dwell_fp!=NULL) 
				{
					...
					for (pl=0; pl<nplns; pl++)
					{
						...
						for (j=0; j<nelems; j++)
						{
							if (dwellp[j]>0) rdwell_c[j] = 1.0f/dwellp[j];
							else rdwell_c[j] = 1.0f;
						}
...
static void usage(char* pgm_name)
{
	...
	fprintf(stderr, "\n");
	fprintf(stderr, "usage1: %s scan_file [norm_file] [-t num_bins] [-q]\n", pgm_name);
	fprintf(stderr, "usage2: %s scan_file1 [scan_file2 ... scan_filen] -o norm_file [-t num_bins][-q]\n", pgm_name);
	fprintf(stderr, "       -t num_bins sets specified radial border bins to 1 (default=3 bins)\n");
	fprintf(stderr, "       -q (quiet) option sets the quality check flag off\n");
	fprintf(stderr, "       span (3 or 9) is automatically determined w.r.t file size\n");
	exit(1);
}
...
int main(int argc, char **argv)
{
	...
	while ((input_count+1) < argc && argv[input_count+1][0] != '-')
	{
		...
		{
			fprintf(stderr, "Number of input files limited to %d\n", MAX_INPUT_FILES);
			exit(1);
		}
	}

	if (input_count==0) usage(argv[0]);
	...
	while (pos<argc)
	{
		if (strncmp(argv[pos], "-o", 2) == 0)
		{
			...
			{
				if (++pos < argc) strcpy(out_file, argv[pos]);
				else usage(argv[0]);
			}
		...
		else if (strncmp(argv[pos], "-t", 2) == 0)
		{
			...
			{
				if (++pos < argc) sscanf(argv[pos],"%d", &border_nbin);
				else usage(argv[0]);
			}
...
//		if (GantryInfo::get("interactionDepth",	ftmp)) def_depth = 10*ftmp;
		if (GantryInfo::get("crystalRadius", ftmp))
		{
			...
			printf("Crystal radius : %g\n", det_dia);
		}
		printf("Crystal Depths (mm) :");
		for (i=0; i<nheads; i++)
		{
			if (!GantryInfo::get("headInfo[%d].type", i, head_type)) break;
			if (head_type != LSO_LSO && head_type != LSO_GSO && head_type != LSO_LYSO) {
				printf("Invalid head %d type : %d \n", i, head_type);
				break;
			}
			if (head_type == LSO_LYSO) crys_depth[i] = 10.0;
			else crys_depth[i] = 7.5; //LSO_LSO or LSO_GSO
			printf(" %g", crys_depth[i]);
		}
		printf("\n");
		...
		if (!uniform_flag)
		{
			printf("Using %g mm for all heads\n", def_depth);
			for (i=0; i<nheads; i++) crys_depth[i] = def_depth;
		}
	}
