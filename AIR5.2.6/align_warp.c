...

{
	...
	unsigned int imodel=0U;		/*menu identified model for registration*/
	unsigned int fmodel=0U;		/*final model*/
			...
			while(current_arg<end_of_args){
				...
				switch(*current_char++){
					...
					case 'l':
						if(current_arg>=end_of_args){
							printf("%s: %d: ",__FILE__,__LINE__);
							printf("An image file name must follow -l\n");
							exit(EXIT_FAILURE);
						}
						lesion_file=*current_arg++;
						break;
