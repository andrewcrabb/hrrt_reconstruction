			...
			if(wi[0]==0.0){
				...
				*/
				if(wi[1]==0.0){
					...
					0 0 0 1
					*/
					if(wr[0]<=0.0 || wr[1]<=0.0 || wr[2]<=0.0){
						return AIR_NONPOSITIVE_ELOGER_ERROR;
					}
					if(fabs(wr[0]-wr[1])<TOOSMALL || fabs(wr[0]-wr[2])<TOOSMALL || fabs(wr[0]-wr[3])<TOOSMALL || fabs(wr[1]-wr[2])<TOOSMALL || fabs(wr[1]-wr[3])<TOOSMALL || fabs(wr[2]-wr[3])<TOOSMALL) fastok=FALSE;
				}
				else{
					/* 	
					R x x x
					...
					*/
					...
					if(fabs(wr[0]-wr[3])<TOOSMALL) fastok=FALSE;
				}
			}
			else{
				a[1][2]=0.0;
				...
				*/		
				...
				if(fabs(wr[2]-wr[3])<TOOSMALL) fastok=FALSE;
			}
		}
	}
	{
