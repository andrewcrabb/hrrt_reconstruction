...
Purpose A usage:
  motion_qc -e em_dyn_file  -n norm [-r ref_frame | -R ref_frame_trues ] [-O] [-t]              
´    - Default reference is first frame with trues=4.0M (-R 4.0)
     - Default start_frame is first frame with trues=0.5M
     - Default end_frame is last frame with trues=0.5M 
...
#define DEF_REF_FRAME_LEN 300 // Default reference frame len in sec
...
static void usage(const char *pgm)
{
  ...
  printf("usage: %s em_file  -n norm [-u mu-map] [-F start_frame[,end_frame]]"
          "      [-r ref_frame -R ref_frame_trues -S ref_frame_sum_trues] "
          "      [-x recon_pgm] [-A alignment_method] [options]\n", pgm);
  printf("        -e : em_file.[l64|dyn], histogrammed if .l64 \n");
  printf("Options: \n");
  ...
  for (int frame=0; frame<num_frames; frame++)
  {
    ...
    else 
    {
      ...
      if (exec) chdir(qc_dir);
        //strcpy(cmd_line,"pgnuplot scatter_qc_00.plt");
...
{
  char *recon_pgm=NULL, *align_pgm="AIR";
  ...
  char line[80];
  float ref_frame_trues=4.0E6f, sum_frame_trues=0.0f, min_frame_trues=2.0E5f;
  int frame=0;
  ...
  em_file = argv[1];
  mu_file[0] = '\0';
  while ((c = getopt (argc-1, argv+1, "n:r:R:F:S:x:D:A:u:a:g:T:Otv")) != EOF) {
    switch (c) {
      case 'n':
        norm_file = optarg;
        break;
      ...
      case 'v' :
        ...
        break;
      case 'A':
        printf("-A option not yet implemented\n");
        exit(1);
      case 'T':
        ...
        break;
      case 'R':
        if (sscanf(optarg,"%g",&ref_frame_trues) != 1) 
        {
          printf("Invalid reference frame trues : %s\n", optarg);
          usage(argv[0]);
        }
        break;
      case 'S':
        if (sscanf(optarg,"%g",&sum_frame_trues) != 1) 
        {
          printf("Invalid reference frame trues : %s\n", optarg);
          usage(argv[0]);
        }
  ...
  *ext++ = '\0';
  ...
  { // ecat reconstructed images file
    ...
    for (frame=0; frame<num_frames && node!=NULL; frame++)
    {
      ...
      if (ref_frame<0 && frame_info[frame].duration>= DEF_REF_FRAME_LEN) ref_frame = frame;
      node = node->next;
  ...
  if (em_ecat_file[0] == '\0')
  {
    ...
    if (start_frame<0) 
    {
      ...
      {
        if (frame_info[frame].trues > min_frame_trues)
    ...
    if (ref_frame<0)
    {
      if (sum_frame_trues>0) 
      {
        double sum=0.0;
        for (frame=0; frame<num_frames && sum<sum_frame_trues; frame++){
          sum += frame_info[frame].trues;
          printf("\n frame_info[%d].trues:%f", frame, frame_info[frame].trues); fflush(stdout);
        }
        ref_frame = frame;  // standardized 0-N countingg
      }
      else 
      {
        for (frame=0; frame<num_frames && ref_frame<0; frame++)
        {
          if (frame_info[frame].trues > ref_frame_trues)
          {
...
#ifdef WIN32
      sprintf(cmd_line,"%s -t %s -n %s -X 128 -o %s -W 0 -I 4 -S 8 -N", 
        recon_pgm, em_dyn_file, norm_file, em_ecat_file);
#else // multithread bug on linux, use 1 thread 
      sprintf(cmd_line,"%s -t %s -n %s -X 128 -o %s -W 0 -I 4 -S 8 -N -T 1",
        recon_pgm, em_dyn_file, norm_file, em_ecat_file);
#endif
    ...
    {  // can't create image files, create scatter QC
      em_ecat_file[0] = '\0';
      if (strlen(mu_file)>0) compute_scatter();
    }
  }
