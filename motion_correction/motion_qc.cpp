...
int main(int argc, char **argv)
{
  ...
  em_file = argv[1];
  mu_file[0] = '\0';
  while ((c = getopt (argc-1, argv+1, "n:r:F:x:D:u:a:L:g:T:R:Otv")) != EOF) {
    switch (c) {
      case 'n':
        norm_file = optarg;
        break;
  ...
  *ext++ = '\0';
  ...
  { // ecat reconstructed images file
    ...
    for (frame=0; frame<num_frames && node!=NULL; frame++)
    {
      ...
      //printf("\n frame_info[%d].start_time:  %d", frame, frame_info[frame].start_time); fflush(stdout);
      printf("\n frame_info[%d].trues: %f (start_time: %d)", frame, frame_info[frame].trues,frame_info[frame].start_time); fflush(stdout);
  ...
  if (em_ecat_file[0] == '\0')
  {
    ...
    printf("\n\n\n num_frames:%d", num_frames); fflush(stdout);
