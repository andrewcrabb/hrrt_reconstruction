...
08jun2009: Removed outputstring code in release mode
06Oct2010: Moved GetMappedBuffer from LMStart to LmAcqControl
           after SendNotificationHandle 
           Added second Acquire call retry after 200msec when first fails
/*********************************************************************/
...
LmAcqControl ()
{
	...
	HANDLE ReadyEvent ;
	LMSTATS lmstats ;
	ULONG bnum, acq_status, *bufr0, *bufr1, *sbufr ;
	...
	if (!Acq.m_Systran)
	{
	  ...
	  if (DP) cout << "CTI_LM AcqControl: Calling Map Buffers..." << endl ;
    if (!(Acq.pFcd->GetMappedBuffer ())) {
      cout << "CTI_LM AcqControl: Error Calling Map Buffers " << GetLastError () << endl ;
      return;
    }
	  if (DP) cout << "CTI_LM AcqControl: Memory buffers mapped..." << endl ;
		...
		while (acq_status != ACQ_STOPPED)
		{
			...
			else // online
			{
				...
				if (DP) cout << "CTI_LM AcqControl: Setting stop condition..." << endl ;
				...
				{
					// ZYB cout << "CTI_LM : LmAcqControl... Inside histogram portion" << endl ;
					histRet = HISTBuf ((char *)sbufr, (int)nbytes, stopFlag) ;
					// ZYB cout << "CTI_LM : LmAcqControl... histRet " << histRet << endl ;
					...
					if (!histRet)
					{
						...
						cout << "CTI_LM : LmAcqControl... Stopping in histo halt" << endl ;
						if (stopFlag < 0) Acq.pFcd->Stop () ;
						acq_status = ACQ_STOPPED ;
			...
			if (DP) cout << "CTI_LM AcqControl: Tagging buffer " << (bnum & 1) << " free..." << endl ;
