...
/********************************************************************
 ...
 08-APR-2010: Added to HRRT User comminity software (Merence Sibomana)
 30-JUL-2010: Divide by Zero bug fix (Merence Sibomana)
/*******************************************************************/
	...
	)
{
	...
	DbgPrint ("PftStopAcquisition: Inside STOP\n") ;
	
	// set the acquisition state to preset
	pDevExt->acqstatus = ACQ_PRESET ;
	...
	)
{
	...
	// Clear the event out
	KeClearEvent ((PKEVENT)pDevExt->EventHandle) ;

	// Check to see if we have to restart
	if (nbufr->state == PENDING)
	{
		// We have to set the event due to the pause
		KeSetEvent ((PKEVENT)pDevExt->EventHandle, 0, FALSE) ;
		...
		// If we are still in the active state restart
	...
	)
{
	...
	if (dmacsr & DMA_CHN0_DONE)
	{
		...
		if ((pDevExt->acqstatus != ACQ_STOPPED) && (pDevExt->acqstatus != ACQ_PRESET))
		{
			if (nevents < dmacnt)
			{
		...
		// Update the total events counter and current buffer count
		pDevExt->TotalEvents += current->events ;
    ...
    )
{
	...
	// Make sure the event handle is valid before continuing
	if (pDevExt->EventHandle)
	{
		// Set the event if we need to
		if (pDevExt->SendEvent) KeSetEvent ((PKEVENT)pDevExt->EventHandle, 0, FALSE) ;
	}

	// If we've hit preset it is time to stop
	if (pDevExt->acqstatus == ACQ_PRESET) pDevExt->acqstatus = ACQ_STOPPED ;
...
{
	LARGE_INTEGER StatsPollTime, CurrentTime, CurrentTime_secs ;
	...
	while (1)
	{
		...
		// new computation - 12mar03-jhr
		totalcounts = pDevExt->TotalEvents = (__int64)current_count ;
		pDevExt->lmstats.total_events = pDevExt->TotalEvents ;
  ...
  // Prevent divide by zero in the driver to avoid the BSOD.
	CurrentTime_secs.QuadPart = pDevExt->ElapsedTime.QuadPart / 10000000 ;
	pDevExt->EventRate = CurrentTime_secs.QuadPart>0? (ULONG)(totalcounts / CurrentTime_secs.QuadPart) : 0UL ;
