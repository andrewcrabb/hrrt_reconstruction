  ...
  if (sFlag) {
    if (stat(osem3dpar->scat_scan_file, &st) == 0) {
      // if (st.st_size == (xr_pixels*views*z_pixels + segmentsize_s9)*sizeof(float)) {
       if (st.st_size == (def_Elements*views*z_pixels + segmentsize_s9)*sizeof(float)) {
        s2DFlag = true;
      } else {
        //if (st.st_size != (xr_pixels*views*segmentsize)*sizeof(float)) {
        if (st.st_size != (def_Elements*views*segmentsize)*sizeof(float)) {
          if (max_g<0 && min_g<0)
          { // check file size
            LogMessage("%s: Invalid file size %d (xr_pixels:%d), expected %d\n", scat_scan_file,
                       st.st_size, xr_pixels, (xr_pixels*views*segmentsize)*sizeof(float));
            exit(1);
          }
        }
      }
  ...
  /* Start processing */
	...
	if (calnormfacFlag) {
		...
		CalculateNormfac();
                ...
                printf("\n\n\n\n ATTENTION: For some reason, if using -T 4 (more than 1 threads), if NormFac is calculated inside recon, output images are flawed. So whenever this is done, we quite the program so the user knows to re-run the code this time using the already-calculated NormFac!!! \n\n\n"); fflush(stdout);
                exit(0);
	} else {
		if(normfac_in_file_flag!=-1)alloc_normfac_infile(weighting,span);
		if(normfac_in_file_flag==1) normfac_in_file_flag=2;
