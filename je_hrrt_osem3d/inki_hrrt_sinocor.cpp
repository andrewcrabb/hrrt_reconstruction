...
const char *sw_version = "HRRT_U 1.0.1";
char sw_build_id[80];
...
//int write_image_header(ImageHeaderInfo *info, int psf_flag, const char *sw_version, const char *sw_build_id);
...
void Output_CorrectedScan()
{
  ...
  for(v=0;v<views/2;v++)
  {
    ...
    for(yr2=0;yr2<segmentsize*2;yr2++) 
      memcpy(imageprj[v][yr2],estimate[yr2],xr_pixels*4);
  }
  ...
  for(theta=0;theta<th_pixels;theta++)
	  ...
	  for( yr=yr_bottom[theta]; yr<=yr_top[theta]; yr++, yr2++)
		  ...
		  // Replace corrected trues by image projection where norm=0 to fill the gaps
		  for (i=0; i<npixels; i++)
			  {
			  if (no[i]< eps){
				  emc[i] = emp[i];
				  if(emc[i]<-1e8) emc[i]=eps;
				  }
			  // else emc[i]=eps;
			  // if(emc[i]<eps) emc[i]=eps;
			  }
...
int main(int argc, char* argv[])
{
  ...
  if (ssinofp != (FILEPTR)0) {
    ...
    if (fstat(fileno(ssinofp), &st) == 0) {
      if (st.st_size == (xr_pixels*views*z_pixels + segmentsize)*sizeof(float)) {
        s2DFlag = true;
      } else {
