...
static void AIR_start_element(void *userData, const char* name, const char **atts)
{
  ...
  case CLASS_REFERENCE:
    if ((v = get_attr("filename", atts))) {
      ...
      if (f_ok == -1)  LOG_EXIT("Reference : invalid filename"); 
      strcpy(air.r_file, filename);
    } else LOG_EXIT("Reference: missing filename");
    if ((v = get_attr("dimension", atts))) {
      if (sscanf(v,"%d,%d,%d", &air.s.x_dim,&air.s.y_dim,&air.s.z_dim) != 3)
        LOG_EXIT("Invalid Reference voxelsize: %s", v);
    }
  ...
  case CLASS_RESLICE:
    if ((v = get_attr("filename", atts))) {
      ...
      if (f_ok == -1)  LOG_EXIT("Reslice: invalid filename"); 
        strcpy(air.r_file, filename);
    } else LOG_EXIT("Reslice: missing filename");
    ...
    if ((v = get_attr("param", atts))) {
      ...
      else LOG_EXIT("Invalid Reslice param: %s");
    }
      ...
      for (j=0; j<4; i++) {
        for (i=0; i<4 && sv!=NULL; i++) {
          if (sscanf(sv,"%g",&air.e[i][j]) != 1) 
            LOG_EXIT("Invalid Reslice matrix: %s (%s)", v, sv); 
          if ((sv=strchr(sv,',')) != NULL) sv++;
        }
        if (i !=4 || j!= 4) 
          LOG_EXIT("Invalid Reslice matrix: %s : too short", v); 
      }
    }
    break;	
...
struct AIR_Air16 *xmldecode_air16(const char *filename)
{
  ...
  XML_Parser parser;
	...
	if (filename == NULL || strlen(filename) == 0)
		LOG_EXIT("Null or blank filename");
	if (stat(filename, &st) < 0)	LOG_EXIT(strerror(errno));
	if (buf_size<(st.st_size+1)) buf_size = st.st_size+1; 
	if ((fp = fopen(filename,"rb")) == NULL) LOG_EXIT(strerror(errno));
	buf = (char*)calloc(1,buf_size);
	if (fread(buf,len, 1, fp) != 1) LOG_EXIT(strerror(errno));
	buf[len+1] = '\0';
	if (strcmp(buf, magic_number) != 0)  LOG_EXIT("Invalid magic number");
  ...
  if ((p = strrchr(current_dir, DIR_SEPARATOR)) == NULL) strcpy(current_dir, ".");
	...
	do
	{
		...
		{
			fclose(fp);
			LOG_EXIT(XML_ErrorString(XML_GetErrorCode(parser)));
		}
	} while (!done);
	fclose(fp);
	free(buf);
