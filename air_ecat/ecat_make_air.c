...
/* Modification history: Merence Sibomana <sibomana@gmail.com> 
   02-MAR-2011: Add -i option to convert .air file pixel sizes
                Corrections for non-isotropic voxel size
*/
...
{
	struct AIR_Air16		air1; // output transformer file
  struct AIR_Air16	  air2; // Input transformer file
	int c, convention=0;
  ...
  else
  {
    ...
    memcpy(air2.s_file, air1.s_file, sizeof(air2.s_file));
	  pixel_size = (float)(min(air2.r.x_size,air2.r.y_size));
	  pixel_size = (float)(min(pixel_size,air2.r.z_size));
    for (i=0; i<4; i++) {
      for (j=0; j<4; j++) mat->data[j*4+i] = (float)air2.e[i][j];
    }
    // scale to istropic voxel size
    scale(mat, (float)air2.r.x_size/pixel_size, 
      (float)air2.r.y_size/pixel_size, (float)air2.r.z_size/pixel_size);
    for (i=0; i<4; i++) {
      for (j=0; j<4; j++) e[i][j] = air2.e[i][j] =  mat->data[j*4+i];
    }
    // Corrections for non-isotropic voxel size
    c[0] = (float)(pixel_size/air1.r.x_size);
    c[1] = (float)(pixel_size/air1.r.y_size);
    c[2] = (float)(pixel_size/air1.r.z_size);
    for (j=0; j<3; j++) e[3][j] = air2.e[3][j] =  mat->data[j*4+3]*c[j];
    for (i=0; i<2; i++) e[i][2] = air2.e[i][2] =  mat->data[2*4+i]/c[i];
    for (j=0; j<2; j++) e[2][j] = air2.e[2][j] =  mat->data[j*4+2]*c[j];
