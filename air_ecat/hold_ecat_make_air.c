...
static void usage(const char *pgm) {
  ...
  fprintf(stderr, "usage: %s -s standard_file -r reslice_file -t yaw(z_rot),pitch(x_rot),roll(y_rot),tx,ty,tz"
    " -o air_file [-N]\n", pgm);
...
main(int argc, char **argv)
{
	...
	memset(air_file,0,128);
	while ((c = getopt (argc, argv, "s:r:t:o:N")) != EOF) {
	switch(c) {
		case 'N' :
	...
	if (air_file[0]==0 || air1.s_file[0]==0 || air1.r_file[0]==0 ||
    tflag==0 ) usage(argv[0]);
	...
	pixel_size = min(air1.r.x_size,air1.r.y_size);
	pixel_size = min(pixel_size,air1.r.z_size);
  ...
  */
  AIR_write_air16(air_file,permission,e,zooming,&air1);
}
