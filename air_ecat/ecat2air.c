...
// Duplicate of same method in interfile.cpp
...
void  matrix_flip(ecat_matrix::MatrixData *data, int x_flip, int y_flip, int z_flip)
{
  ...
  if (z_flip) {
    ...
    void *tmp = (void *) malloc(npixels * elem_size);
  ...
  for (z = 0; z < data->zdim; z++) {
    if (y_flip)
      flip_y(plane_in, data->data_type, data->xdim, data->ydim);
    if ( x_flip) {
      for (y = 0; y < data->ydim; y++) {
        void *line = plane_in + y * data->xdim * elem_size;
