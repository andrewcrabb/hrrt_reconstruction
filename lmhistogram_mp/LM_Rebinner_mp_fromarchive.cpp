 ...
 * t_max_rd is set to (span-1)/2.
 * Returns 1 if OK and 0 if gm328.ini not found or if a key is not found.
 */
// int init_rebinner(int &span, int &t_max_rd)
int init_rebinner(int &t_span, int &t_max_rd, char *t_lut_file) {
  ...
  if (GantryInfo::load(model_number) > 0) {
    ...
    if (t_span = = 0) { // Transmission mode
      ...
      if (LR_type  > 0 ) {
        ...
        cout << "Low Resolution mode: t_span changed to " << t_span << endl;
      }
      t_max_rd = (t_span - 1) / 2;
    }
  } else {
