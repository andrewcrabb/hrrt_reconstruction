...
/*
	int convert_span9(short *sino, int maxrd) 
	converts in place span3 to span 9 sinogram
*/
int convert_span9(short *sino, int maxrd, int nrings)
{
	int iseg=0, seg=0, span3=3, span9=9;
	int nseg = (2*maxrd+1)/span3;  // span 3 # egments
	int nplns0 = 2*nrings -1;		// Segment 0 #planes
	int ntotal = nseg*nplns0 - (nseg-1)*(span3*(nseg-1)/2+1);

	int iseg_9=0, nseg_9 = (2*maxrd+1)/span9;
	int offset = 0;
	...
	while (iseg_9<nseg_9)
	{
      ...
      for (j=0; j<nvoxels; j++) out_p[j] += in_p[j];
		...
		else
		{
			...
			in_p = sino + span3_pos*npixels;
			nvoxels = npixels*nplns[iseg];
			for (j=0; j<nvoxels; j++) out_data_neg[j] = in_p[j];
			iseg++;
			...
			in_p = sino + span3_pos*npixels;
			nvoxels = npixels*nplns[iseg];
			for (j=0; j<nvoxels; j++) out_data_pos[j] = in_p[j];
			iseg++;
