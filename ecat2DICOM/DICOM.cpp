		 ...
		 DICOMMap &dcm_map, int &DICOM10_flag)
{
  ...
  if ((count = fread(buf,1,st.st_size,fp)) != st.st_size) {
    if (count<0) perror(filename);
    else fprintf(stderr,"Read fail : only %d of %d  bytes\n",
		 count, st.st_size);
  }
  fclose(fp);
...
int main(int argc, char **argv)
{
  ...
  if (DICOM_open(argv[1], buf, buf_size, dcm_map, DICOM10_flag) > 0) {
    if (DICOM10_flag) printf("group,elem \t pos \t len \t type\n");
    else printf("group,elem \t pos \t len\n");
   for (i=dcm_map.begin(); i!=dcm_map.end(); i++) {
       for (j=i->second.begin(); j!=i->second.end(); j++) {
	 if (DICOM10_flag) {
	   printf("%x,%x \t %d \t %d \t %s",i->first, j->first,
		  j->second.offset,j->second.len, DICOM10_type(j->second.type));
  	   if (j->second.type==S2B2("UL")) printf(" %d\n", *(int*)(buf+j->second.offset));
  	   if (j->second.type==S2B2("ST")) printf(" %s\n", (char*)(buf+j->second.offset));
  	   if (j->second.type==S2B2("SL")) printf(" %s\n", (char*)(buf+j->second.offset));
  	   if (j->second.type==S2B2("CS")) printf(" %s\n", (char*)(buf+j->second.offset));
	   printf("\n");
	 } else {	   
	   printf("%x,%x \t %d \t %d \t %s\n",i->first, j->first,
		  j->second.offset,j->second.len);
	 }
       }
   }
   return 1;
