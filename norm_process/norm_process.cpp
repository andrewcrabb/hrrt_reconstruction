...
int main(int argc, char **argv)
{
  ...
  {
    lm_file = argv[1];
    if (strlen(drive)+strlen(dir)>0) sprintf(rdwell_path,"%s%s",drive,dir);
    else strcpy(rdwell_path,".");
  }
	else if (_stricmp(ext,".fs") == 0) fs_file = argv[1];
	else if (_stricmp(ext,".ce") == 0) ce_file = argv[1];
	else usage(argv[0]);
...
#endif
  if (argc>2)
    while ((c=getopt( argc-1, argv+1, "v:p:s:g:G:o:O:k:c:d:L:l:R:T:b")) != -1)
    {
    switch(c) {
			// -v set verbose mode
