/******************************************************************************
  ...
  Copyright (c) 2006,2007 Turku PET Centre
  ...
  Modification history:
   ...
   2008-02-21: version 2.0; replace vector arguments by pointer (MS)
 ...
 */
int interfile_read(char *headerName, char *searchWord, char *returnValue,
                   char *errorMessage) {
  short int  i, pos;
  ...
  FILE       *interfileHeader;
                                                        ...
                                                        /* initialise strings */
  for (i=0;i<RETURN_MSG_SIZE;i++) returnValue[i] = '\0';
  for (i=0;i<ERROR_MSG_SIZE;i++) errorMessage[i] = '\0';

                                         /* open interfile header for reading */
//  if ((interfileHeader = fopen(headerName,"r"))==NULL) {
  interfileHeader = fopen(headerName,"r");
  if (!interfileHeader) {
      perror(headerName);
