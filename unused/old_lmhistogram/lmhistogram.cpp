...
static FILE_IN file_in = {INVALID_HANDLE_VALUE, 512};	// input listmode file pointer
static FILE_IN *in=NULL;
...
HANDLE sinowrite_ok = INVALID_HANDLE_VALUE;

HANDLE h_threads[6];    // maximum 6 threads (1 reader, 1 to 4 rebinners, 1 writer or 1 histogrammer
...

{
	...
	{ // HRRT
		...
		{ // emission
			...
			if (hist_mode == 1)
			{    // prompts and delayed
#ifndef HRRT_REBINNER_LUT
        if (span == 3) 
				{
					sinogram_subsize = m_nprojs * m_nviews*nsinos[9]; // delayed in span 9
				}
				else sinogram_subsize = sinogram_size;
#else
	...
	{ // HRRT
		if (sino == NULL) sino = (T *)calloc(sinogram_size+sinogram_subsize,r_size);
		else memset(sino, 0, (sinogram_size+sinogram_subsize)*r_size);
...
void start_reader_thread()
{
	// Create sinogram file write synchronization event
	sinowrite_ok = CreateEvent(NULL, TRUE, FALSE, "sino write");
	if (sinowrite_ok == NULL)
	{
		fprintf(stderr, "lm64_reader: Error creating l64 container synchronization event");
		exit(1);
	}
	// 
    ...
    memcpy(L64EventPacket::frame_skip, skip, nframes*sizeof(int));
		L64EventPacket::container_ok = CreateEvent(NULL, TRUE, FALSE, "l64 container");
		if (L64EventPacket::container_ok == NULL)
		{
			fprintf(stderr, "lm64_reader: Error creating l64 container synchronization event");
			exit(1);
		}
		h_threads[0] = (HANDLE)_beginthread(lm64_reader,0,in_fname);
    // Wait for 64bit buffer while file_status OK to support event skip
		while (lm64_consumer_wait() != WAIT_OBJECT_0)
		{
			if ( !(L64EventPacket::file_status&L64EventPacket::ready) )
			{
				fprintf(stderr, "lm64_reader thread startup failed\n");
				exit(1);
			}
		}
	}
	...
	{
		if ((L32EventPacket::container_ok = CreateEvent(NULL, TRUE, FALSE, "l32 container")) == NULL)
		{
			fprintf(stderr, "lm32_reader: Error creating l32 container synchronization event");
			return;
		}
		h_threads[0] = (HANDLE)_beginthread(lm32_reader,0,in_fname);
		if (lm32_consumer_wait() != WAIT_OBJECT_0)
		{
			if (L32EventPacket::file_status == L32EventPacket::unknown)
			{
				fprintf(stderr, "lm32_reader thread startup failed\n");
				exit(1);
			}
			// else read finished before getting here
		}
	}
}
...
inline void start_rebinner_thread() // Start thread and wait until it is ready
{
  ...
  CString msg;
	if ((L32EventPacket::container_ok = CreateEvent(NULL, TRUE, FALSE, "l32 container")) == NULL)
	{
		log_message("lm64_rebinner: Error creating l32 container synchronization event", 2);
		exit(1);
	}
...
//		dcf2 = ((float)log(2.0) * duration[frame]) / (decay_rate * (1.0 - (float)pow(2,-1.0*duration[frame]/decay_rate)));
	...
	case 1: // Prompts and delayed: trues is second if short type
		...
		if (span == 3)
		{
			printf ("Converting prompt sino to span9\n");
			convert_span9(ssino, max_rd, 104);
			hdr.WriteTag("axial compression",9);
			hdr.WriteTag("matrix size [3]",nsinos[9]);
		}
		printf ("computing trues as prompts - delayed\n");
		if (span==3)
      for (i=0; i<span9_sino_size; i++) ssino[i] =  ssino[i]-delayed[i];
    else for (i=0; i<sino_size; i++) ssino[i] =  ssino[i]-delayed[i];
#else
 ...
 * Log error and exit when an I/O error is encountered.
 */
static void p39_write_tx(short *sino,  int sino_size, CHeader &p39_mhdr)
{
	CHeader hdr;
	char tmp[64];
	int bucket=0, num_buckets = 10;
	// create file
	if ((out2=fopen(outfname_p39_tx,RW_MODE)) == NULL)
	{
		perror(outfname_p39_tx);
		exit(1);
	}
	// create the output header file
	char *data_file = strrchr(outfname_p39_tx,'\\');
	if (data_file == NULL) data_file = outfname_p39_tx;
	else data_file++;
	hdr.WriteTag("image data byte order","LITTLEENDIAN");
	hdr.WriteTag("%CPS data type", "sinogram subheader");
	hdr.WriteTag("!PET data type", "Transmission");
	hdr.WriteTag("!name of data file",data_file);
	hdr.WriteTag("!image duration (sec)", scan_duration);
	hdr.WriteTag("image relative start time (sec)",0);
	hdr.WriteTag("data format", "sinogram");
	hdr.WriteTag("number format", "signed integer");
	hdr.WriteTag("!number of bytes per pixel",  2);
	hdr.WriteTag("number of dimensions",3);
	...
	hdr.WriteTag("start horizontal bed position (mm)", 0.0);
	hdr.WriteTag("start vertical bed position (mm)", 0.0);
	hdr.WriteTag("number of scan data types", 1);
	hdr.WriteTag("scan data type description [1]", "corrected prompts");
	hdr.WriteTag("%axial compression", 7);
	hdr.WriteTag("%maximum ring difference",3);
	hdr.WriteTag("%number of segments", 1);
	hdr.WriteTag("%segment table", "{239}");
//	hdr.WriteTag("decay correction factor",1.0);
//	hdr.WriteTag("decay correction factor2",1.0);
	hdr.WriteTag("total prompts",total_tx_prompts());
	hdr.WriteTag("%total randoms",total_tx_randoms());
	hdr.WriteTag("%Total Net Trues",total_tx_prompts() - total_tx_randoms());
	hdr.WriteTag("%number of bucket-rings ", num_buckets);
	int tot_singles = 0;
	for (bucket = 0; bucket < num_buckets ; bucket++)
		tot_singles += singles_rate(bucket);
...
//	hdr.WriteTag("Dead time correction factor",GetDTC(LLD, tot_singles/num_buckets));
	...
	printf("sinogram size: %d\n",sino_size);
	printf("element size: %d\n", elem_size);
	printf("Writing TX Sinogram: %s\n", outfname_p39_tx);
	int tx_size =  nprojs * nviews * P39_nsinos[0];
	sprintf(outfname_hdr,"%s.hdr", outfname_p39_tx);
	if ((count = fwrite(sino+sino_size, tx_size,elem_size,out2)) == (int)elem_size)
	{
		sprintf(outfname_hdr,"%s.hdr", outfname_p39_tx);
		hdr.WriteFile(outfname_hdr);
	} else 	printf("Error writing %s\n", outfname_p39_tx);
	if (nframes > 1)
	{  // One TX for all frames : Write TX mhdr
		sprintf(tmp, "{1000000000,%s.hdr,%s}",data_file,data_file);
		p39_mhdr.WriteTag("%data set [2]", tmp);
		strcpy(fname_p39_mhdr, outfname_p39_tx);
		char *ext = strrchr(fname_p39_mhdr,'.');
		if (ext != NULL) strcpy(ext,".mhdr");
		else strcat(fname_p39_mhdr, ".mhdr");
		p39_mhdr.WriteFile(fname_p39_mhdr, 1);
...
int main(int argc, char* argv[])
{
  ...
  sprintf(sw_build_id,"%s %s", __DATE__, __TIME__);
	...
	msg.Format("%s %s", argv[0], in_fname);
	log_message(msg);
	if (_access(in_fname,4) != 0)
	{ 
		msg.Format("%s: File not found", in_fname);
		log_message(msg,2);
		return 1;
	}
	if (hdr.OpenFile(in_fname_hdr) == 1)
	{
		msg.Format("Using header: [%s]", in_fname_hdr);
		log_message(msg);
		hdr.CloseFile();
	}
	else 
	{
		msg.Format("header [%s] not found", in_fname_hdr);
		log_message(msg);
	}
...
//		if (l64_flag) init_rebinner(span, max_rd); 
		start_reader_thread();
		lmscan(in,out_hc,&scan_duration);
		fclose(out_hc);
...
//			cout << hc[index].time << " " << hc[index].randoms_rate << " " << hc[index].trues_rate << " " << hc[index].singles << endl;
	...
	else if (out_l32)
	{
		if (!l64_flag)
		{
			cerr << "Input file must be 64-bit listmode for 32-bit listmode output" << endl;
			exit(1);
		}
		
		strcpy(outfname_hdr,out_fname);
		strcat(outfname_hdr,".hdr");
		strcpy(outfname_hc,out_fname);
		outfname_hc[strlen(outfname_hc) - 2] = '\0';  // remove the .s from the end of the name
		strcat(outfname_hc,"_lm.hc");
		cout << "Using header: [" << in_fname_hdr << "]" << endl;

		// init rebinner with specified span and max_rd
		init_rebinner(span, max_rd); 
    msg.Format("using rebinner LUT %s",rebinner_lut_file);
    log_message(msg);

		//
		// Start threads and wait for all to finish
		//
		start_reader_thread();			// start lm64_reader thread and wait until ready
//		L64EventPacket::hc_out = stdout;
		start_rebinner_thread();		// start lm64_rebinner thread  and wait until ready
		// l32 container ready at this point, start lm32 writer thread and
		// wait for all threads completion
		cout << "Starting lm_writer" << endl;
		h_threads[1] = (HANDLE) _beginthread(lm32_writer,0,out_fname); 
		cout << "lm_writer started" << endl;
		// Wait for all threads to complete
		if (WaitForMultipleObjects(3, h_threads, true,	INFINITE ) == WAIT_FAILED)
		{  // !!!!always comes here even though all threads finished OK		
//			cerr << "32-bit listmode output failed" << endl;
//			exit(1);			
		}

		//
		// Build the headers
		hdr.WriteTag("!name of data file",out_fname);
		hdr.WriteTag("!LM rebinner method","2");
		hdr.WriteTag("axial compression",span);
		hdr.WriteTag("maximum ring difference", max_rd);
		if (!hdr.Readchar("!PET data type",datatype,32))
		{
			if (strcasecmp(datatype,"transmission") == 0) hist_mode = 7;
		}
		// Write header
		hdr.WriteFile(outfname_hdr);
	}
	else if (out_l64)
	{
		if (!l64_flag)
		{
			cerr << "Input file must be 64-bit listmode for 64-bit listmode output" << endl;
			exit(1);
		}
		...
		//
		start_reader_thread();			// start lm64_reader thread and wait until ready
		h_threads[1] = (HANDLE) _beginthread(lm64_writer,0,out_fname); 
		// Wait for all threads to complete
		if (WaitForMultipleObjects(2, h_threads, true,	INFINITE ) == WAIT_FAILED)
		{   // !!!!always comes here even though all threads finished OK		
//			cerr << "64-bit listmode output failed" << endl;
//			exit(1);			
		}
   ...
   // Check start countrate and set first frame skip 
    if (start_countrate> 0) 
    {
        ...
        if (skip_msec>0) 
        {
          ...
          msg.Format("%d sec will be skipped to start with %d trues/sec countrate", 
		...
		while (frame < nframes)
		{
			...
			if (elem_size == 2)
			{
			    relative_start = histogram<short>(in,ssino, delayed, sinogram_size, duration[frame], out_hc);
			}
			else
			{
			    relative_start = histogram<char>(in, bsino, delayed, sinogram_size, duration[frame], out_hc);
			}
			...
			L32EventPacket::file_status |= L32EventPacket::hold;
			if (check_model(model_number,MODEL_HRRT)) write_sino(sinogram, sinogram_size, hdr, frame);
			else p39_write_sino(sinogram, delayed, sinogram_size, hdr, frame);
			close_files();
			if (l64_flag && hist_mode != 7) write_coin_map(outfname_sino);
			L64EventPacket::file_status = L64EventPacket::file_status & (~L64EventPacket::hold);
			L32EventPacket::file_status = L32EventPacket::file_status & (~L32EventPacket::hold);
			SetEvent(sinowrite_ok);

			if (nframes>1) msg.Format("Frame time histogrammed = %d sec", duration[frame]);
			else msg.Format("Time histogrammed = %d sec", duration[frame]);
			log_message(msg);
			frame++;
		}
#ifndef HRRT_REBINNER_LUT
		if (!check_model(model_number,MODEL_HRRT))
      p39_write_tx((short*)sinogram, sinogram_size, hdr);
#endif
		if (nframes>1) 
		{
			msg.Format("Total time histogrammed = %d secs", current_time);
			log_message(msg);
		}
  ...
  clean_segment_info();
	...
	msg.Format("Processing time = %g sec",0.001f*(t1-t0));
	log_message(msg);
	return 0;
}
