...
/* HRRT User Community modifications
   ...
   9-July-2009 : Add timestamp in millisec to log file
*/
	...
	while (count < length)
	{
		...
		if ( dwCount == 1 )
		{
			if ( c == '\n' )   // is it a command terminated
			{
				...
				if (out_file && pDHI.m_LogFlag)
				{
					fprintf( out_file, "%02d:%02d:%02d.%03d Receive Good: %s\n", 
						st.wHour, st.wMinute, st.wSecond, st.wMilliseconds, buffer);	 
				}
				if (out_file != NULL)
					fclose (out_file);
			...
			if (time(0) > finish_time) // did timed out?
			{
				...
				if (out_file && pDHI.m_LogFlag)
				{
					fprintf( out_file, "%02d:%02d:%02d.%03d Receive Timeout\n", 
