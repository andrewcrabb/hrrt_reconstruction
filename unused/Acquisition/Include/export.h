...
/*
 * Memory is currently limited to 2^31 on most platforms. To use 64-bit
 * addressing on systems that can do it, define IDL_MEMINT_64.
 *
...
#ifdef IDL_MEMINT_64
/*
 * On most 64-bit machines, sizeof(IDL_STRING) is 16 bytes because the
 * presence of a pointer causes the compiler to round up the size to
 * an 8-byte boundary. We can therefore make the length larger than a
 * short without using any additional memory.
 */
...
#if defined(unix) && !defined(HPUX_11)
#ifndef AIX
#include <strings.h>		/* For b* routines */
#endif
#else
#ifndef bcopy
...
#if 0
#define IDL_LMGR_CANCOMPILE				0x100
#define IDL_LMGR_CANRESTORE				0x200
#endif
...
#define IDL_INIT_VAX_FLOAT 1024   /* VMS-only: Cause the /VAX_FLOAT
				     ...
				     FALSE. This lets VAX dependent code
				     run as if VMS/IDL still used VAX
				     floating point. */

#define IDL_INIT_LMQUEUE	2048	/* For FLEXlm licensing only, cause
                     license manager to queue for a license if floating
                     licenses exist but are currently not available. */

#define IDL_INIT_GENVER	4096	/* cmd line option used to request genver
                     mode. command line option available only on unix & vms */
...
#define IDL_F_ISATTY        ((IDL_LONG64) 1)   /* Is a terminal */
				/* Is a Graphical User Interface */
#define IDL_F_ISAGUI	    ((IDL_LONG64) 2)
#define IDL_F_NOCLOSE       ((IDL_LONG64) 4)   /* Don't let user close */
				/* Use more(1) like pager for fmt output */
#define IDL_F_MORE          ((IDL_LONG64) 8)
#define IDL_F_XDR           ((IDL_LONG64) 16)	/* Is a XDR file */
#define IDL_F_DEL_ON_CLOSE  ((IDL_LONG64) 32)	/* Delete on close */
#define IDL_F_SR            ((IDL_LONG64) 64)	/* Is a SAVE/RESTORE file. */
				/* File has opposite byte order than
				   current system. */
#define IDL_F_SWAP_ENDIAN   ((IDL_LONG64) 128)
				/* Binary float and double are in VAX
				   F and D format. Implies that file is
				   little endian. */
#define IDL_F_VAX_FLOAT	    (((IDL_LONG64) 1) << 8)
				/* If a normal file, file is in compressed
				   gzip format. If IDL_F_SR, then file
				   contains zlib compressed data. */
#define IDL_F_COMPRESS	    (((IDL_LONG64) 1) << 9)
				/* Unformatted f77(1) I/O */
#define IDL_F_UNIX_F77      (((IDL_LONG64) 1) << 10)
				/* File is a pipe to a child process */
#define IDL_F_UNIX_PIPE     (((IDL_LONG64) 1) << 11)
				/* Call read(2) and write(2) directly
				   and allow short transfers. */
#define IDL_F_UNIX_RAWIO    (((IDL_LONG64) 1) << 12)
#define IDL_F_UNIX_NOSTDIO  IDL_F_UNIX_RAWIO   /* Old name for rawio */
				/* It's a device/special file */
#define IDL_F_UNIX_SPECIAL  (((IDL_LONG64) 1) << 13)
				/* Fixed length records */
#define IDL_F_VMS_FIXED     (((IDL_LONG64) 1) << 14)
				/* Variable length records */
#define IDL_F_VMS_VARIABLE  (((IDL_LONG64) 1) << 15)
				/* FORTRAN segmented var len records */
#define IDL_F_VMS_SEGMENTED (((IDL_LONG64) 1) << 16)
#define IDL_F_VMS_STREAM    (((IDL_LONG64) 1) << 17) /* Stream file */
				/* When reading a non-stream file via
				   ...
				   in this case. */
#define IDL_F_VMS_STREAM_STRICT (((IDL_LONG64) 1) << 18)
				/* RMS Block Mode access */
#define IDL_F_VMS_RMSBLK    (((IDL_LONG64) 1) << 19)
				/* RMS block mode files are created with
				   FIXED length 512 byte records. This bit
				   indicates that the RMS UNDEFINED record
				   type should be used. */
#define IDL_F_VMS_RMSBLKUDF  (((IDL_LONG64) 1) << 20)
#define IDL_F_VMS_INDEXED    (((IDL_LONG64) 1) << 21)   /* Indexed file */
				/* Send to SYS$PRINT on close */
#define IDL_F_VMS_PRINT      (((IDL_LONG64) 1) << 22)
				/* Send to SYS$BATCH on close */
#define IDL_F_VMS_SUBMIT     (((IDL_LONG64) 1) << 23)
				/* Truncate file allocation on close*/
#define IDL_F_VMS_TRCLOSE    (((IDL_LONG64) 1) << 24)
				/* CR/LF carriage control */
#define IDL_F_VMS_CCLIST     (((IDL_LONG64) 1) << 25)
				/* FORTRAN style carriage control */
#define IDL_F_VMS_CCFORTRAN  (((IDL_LONG64) 1) << 26)
				/* Explicit carriage control */
#define IDL_F_VMS_CCNONE     (((IDL_LONG64) 1) << 27)
#define IDL_F_VMS_SHARED     (((IDL_LONG64) 1) << 28)   /* Shared access */
				/*Supersede existing version on open*/
#define IDL_F_VMS_SUPERSEDE  (((IDL_LONG64) 1) << 29)
				/* Use stdio to perform I/O instead of the
				   native OS API. This is primarily for people
				   linking stdio based code to IDL, and is not
				   otherwise useful */
#define IDL_F_STDIO          (((IDL_LONG64) 1) << 30)
				/* File is an internet TCP/IP socket */
#define IDL_F_SOCKET	     (((IDL_LONG64) 1) << 31)
				/* Macintosh file should have BIN creator
				   type instead of TEXT. */
#define IDL_F_MAC_BINARY     (((IDL_LONG64) 1) << 32)
 ...
 */
#define IDL_F_DOS_NOAUTOMODE ((IDL_LONG64) 0)
#define IDL_F_DOS_BINARY     ((IDL_LONG64) 0)
...
extern void IDL_CDECL IDL_TerminalRaw IDL_ARG_PROTO((int to_from, int
        fnin));
extern void IDL_CDECL IDL_FileExpandPath IDL_ARG_PROTO((char *path));
...
extern void IDL_CDECL IDL_GetTmpDirPath IDL_ARG_PROTO((char *path));
...
extern int IDL_CDECL IDL_FileOpen IDL_ARG_PROTO((int argc, IDL_VPTR
        argv[], char *argk, int access_mode, IDL_LONG64 extra_flags,  int
        longjmp_safe, int msg_attr));
