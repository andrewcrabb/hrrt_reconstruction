//==========================================================================
// File			:	RingModel921.h  (EXACT with no CT)
...
//
#ifndef MODEL921_CLASS_H
#define MODEL921_CLASS_H

#ifdef WIN32
#ifdef GM_EXPORTS
#define MODEL921_API __declspec(dllexport)
#else
#define MODEL921_API __declspec(dllimport)
#endif
#endif
...
//
// CRingModel921 derived class
//

class CRingModel921 : public CModel {
public:
	CRingModel921();
	virtual ~CRingModel921();

	// access functions
	virtual long model() const { return 921; }
	virtual char* mName();
	void calculate();
	...
	virtual long nBlocks() const { return (_nblocks != BADINIKEY) ? _nblocks : 4; }
	...
	virtual long bktDbSize() const { return (_bktDbSz != BADINIKEY) ? _bktDbSz : 1536; }
	virtual long bktDBStart() const { return (_bktDBSt != BADINIKEY) ? _bktDBSt : 61440; }
	...
	virtual long defLLD() const { return (_LLD != BADINIKEY) ? _LLD : 250; }
	...
	virtual long defPreInjTxLLD() const { return (_preInjTxLLD != BADINIKEY) ? _preInjTxLLD : 250; }
	virtual long defPostInjTxLLD() const { return (_postInjTxLLD != BADINIKEY) ? _postInjTxLLD : 250; }
	...
	virtual double defTransFOV() const { return (_defTransFOV != BADINIKEY) ? _defTransFOV : 58.30;}
	...
	virtual double planeSeparation() const { return (_plnSep != BADINIKEY) ? _plnSep : 0.3375f; }
	virtual double binSz() const { return (_binSize != BADINIKEY) ? _binSize : 0.3375f; }
	...
	virtual long timeCorrectionBits() const { return (_timeCorrBits != BADINIKEY) ? _timeCorrBits : 2; }
	virtual long maxCodePg() const { return (_maxCodePage != BADINIKEY) ? _maxCodePage : 0; }
	virtual long minElems() const { return (_minElems != BADINIKEY) ? _minElems : 0; }
	...
	virtual double maxScanLen() const { return (_maxScanLen != BADINIKEY) ? _maxScanLen : 236.200; }
