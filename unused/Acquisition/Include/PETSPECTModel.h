...
//
#ifndef MODELPETSPECT_CLASS_H
#define MODELPETSPECT_CLASS_H

#ifdef WIN32
#ifdef GM_EXPORTS
#define MODELPETSPECT_API __declspec(dllexport)
#else
#define MODELPETSPECT_API __declspec(dllimport)
#endif
#endif
...
//
// CPETSPECTModel derived class
//

class CPETSPECTModel : public CModel {
public:
	CPETSPECTModel();
	virtual ~CPETSPECTModel();

	// access functions
	virtual long model() const { return 311; }
	virtual char* mName();
	...
	virtual long defElements() const { return (_elements != BADINIKEY) ? _elements : 0; }
	virtual long totSinograms() const { return (_totSino != BADINIKEY) ? _totSino : 0; }
	...
	virtual long numberPolySides() const { return (_polySides != BADINIKEY) ? _polySides : 2; }
	virtual long coincWindow() const { return (_coinWnd != BADINIKEY) ? _coinWnd : 6; }
	virtual char* coincProcName();
	virtual long numberPanels() const { return (_nheads != BADINIKEY) ? _nheads : 2; }
	...
	virtual long cfdIter() const { return (_cfditer != BADINIKEY) ? _cfditer : 3; }
	...
	virtual double interacDepth() const { return (_intDepth != BADINIKEY) ? _intDepth : 1.2; }
	virtual double crystRad() const { return (_crystalRad != BADINIKEY) ? _crystalRad : 40.0; }
	virtual char* layerMat(long);
	virtual long numberCrystaLayers() const { return (_crystalLayer != BADINIKEY) ? _crystalLayer : 2; }
	...
	virtual long timeAlignIter() const { return (_timeAlignIter != BADINIKEY) ? _timeAlignIter : 1; }

	virtual long txCFDIter() const { return (_txcfditer != BADINIKEY) ? _txcfditer : 5; }
	virtual long txCFDSt() const { return (_txcfdstart != BADINIKEY) ? _txcfdstart : 60; }
	virtual long resetWaitDur() const { return (_resetWaitDuration != BADINIKEY) ? _resetWaitDuration : 45; }
