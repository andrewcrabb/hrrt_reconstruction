//==========================================================================
// File			:	RingModel966.h  (HR++ with no CT)
...
//
#ifndef MODEL966_CLASS_H
#define MODEL966_CLASS_H

#ifdef WIN32
#ifdef GM_EXPORTS
#define MODEL966_API __declspec(dllexport)
#else
#define MODEL966_API __declspec(dllimport)
#endif
#endif
...
//
// CRingModel966 derived class
//

class CRingModel966 : public CModel {
public:
	CRingModel966();
	virtual ~CRingModel966();

	// access functions
	virtual long model() const { return 966; }
	virtual char* mName();
	void calculate();
	...
	virtual long nBlocks() const { return (_nblocks != BADINIKEY) ? _nblocks : 12; }
	...
	virtual long bktDbSize() const { return (_bktDbSz != BADINIKEY) ? _bktDbSz : 5632; }
	...
	virtual long bktsPerRing() const { return (_bktsPerRing != BADINIKEY) ? _bktsPerRing : 36; }
	virtual long bktOutputChannels() const { return (_bktOutChnl != BADINIKEY) ? _bktOutChnl : 1; } // single channel=1, dual channel=2
	virtual long bktsInCoinc() const { return (_bktsInCoinc != BADINIKEY) ? _bktsInCoinc : 19; }
	...
	virtual long transBlks() const { return (_tranBlk != BADINIKEY) ? _tranBlk : 2; }
	virtual long axialBlks() const { return (_axialBlk != BADINIKEY) ? _axialBlk : 6; } 
	...
	virtual double maxScanLen() const { return (_maxScanLen != BADINIKEY) ? _maxScanLen : 243.20; }
	...
	virtual double vBedStp() const { return (_vBedStep != BADINIKEY) ? _vBedStep : 1.0f; }
	...
	virtual double maxBedTrav() const { return (_bedTravel != BADINIKEY) ? _bedTravel : 2199.20; }
