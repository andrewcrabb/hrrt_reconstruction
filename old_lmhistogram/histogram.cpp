...
------------------------------------------------
   ...
   Modification history:
          ...
          11-JUN-2010: Handle DOI error in TX listmode (avoid crash for LORs not in LUT)
*/
#include "stdafx.h"
#include <stdio.h>
#include <io.h>
...
long process_tagword(long tagword, long duration, FILE *out_hc)
{
	...
	if ((tagword & 0xE0000000) == 0x80000000)// timetag found
	{
		...
		{ 
			CString msg;
			msg.Format("Missing timetag : %d-%d = %d msec", 
...
inline void load_buffer_32(FILE_IN *in, unsigned * &buf, int &nevents)
{
 ...
 * 3="I/O error"
 */
static int next_event_32(FILE_IN *in, Event_32 &cew, int scan_flag)
{
   unsigned event, tag;
   ...
   if (!nevents) {
	  load_buffer_32(in,listbuf,nevents);
      if (nevents<=0) 
	  {
		  if (GetLastError() != NO_ERROR) return -2;
		  return -3;
	  }
      listptr = listbuf;
   }
 ...
 */
static int goto_event_32(FILE_IN *in, int target)
{
	unsigned event, tag;
...
inline void load_buffer_64(FILE_IN *in, unsigned * &buf, int &nevents, int &nsync)
{
/*
 ...
 * Logs an error and retun 0 otherwise.
 */
static int goto_event_64(FILE_IN *in, int target)
{
	...
	while (!terminate)
	{
		if ((nevents-(nsync+1)/2) == 0) {
		   load_buffer_64(in,listbuf,nevents,nsync);
		   if (nevents<=0)
		   {
			   if (GetLastError() == NO_ERROR)
			   {
 ...
 * 3="I/O error"
 */
static int next_event_64(FILE_IN *in, Event_32 &cew, int scan_flag)
{
...
*/
int goto_event(FILE_IN *in, int target)
{
	if (!timetag_processing && target==0) return 1;
	return goto_event_32(in, target);
}
...
//					cout << "TX pos: " << tx_source.head << "," << tx_source.x << "," << tx_source.y << ": " << tx_mock.timer << "msec" << endl;
		...
		default: // event: 0=prompt, 1=delayed
         ...
         if (hist_mode != 7 && (doia+doib)>2)
         {
             if (verbose)
               cerr <<  current_time <<": Invalid crystal DOI fro EM event(" <<
               doia << "," << ay <<") (" << bx <<"," <<by <<")"<< endl;
             error_flag++;
         }
 ...
 */
template <class T> int histogram(FILE_IN *in, T *sino, char *delayed,
								 int sino_size, int &duration, FILE *out_hc)
{
  ...
  int tx_sino_size = m_nprojs*m_nviews*(2*NYCRYS-1);
	...
	switch (hist_mode)
	{
		...
		case 2: // prompts only
			while (next_event_32(in,cew,0) == 0)
			{
...
template int histogram(FILE_IN *, char *sino, char *delayed, int sino_size, int& duration, FILE* out_hc);
template int histogram(FILE_IN *, short *sino, char *delayed, int sino_size, int& duration, FILE* out_hc);

static void lmscan_32(FILE_IN *in, FILE *out, long *duration)
{
...
static void lmscan_64(FILE_IN *in, FILE *out, long *duration)
{
	long prev_time=0, time = 0;
	long prompts = 0, randoms=0, total_singles=0;
	long max_plane = 0;
...
{
	if (!l64_flag) lmscan_32(in, out, duration);
	else lmscan_64(in, out, duration);
}
