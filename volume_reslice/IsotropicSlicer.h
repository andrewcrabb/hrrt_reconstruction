...
//extern int debug_level;
  ...
  if (area_x>volume->xdim*volume->pixel_size)
	  sx = volume->xdim;
  if (area_y>volume->ydim*volume->y_size)
	  sy = volume->ydim;
  //////////////////////////////////////////////////
  ...
  transf_z_slice(dest, z, sx, sy, pixel_size); // use mid slice
                                         ...
                                         float area_z, float pixel_size) const
{
  ...
  if (area_y>volume->ydim*volume->y_size)
	  sy = volume->ydim;
  if (area_z>volume->zdim*volume->z_size)
	  sz = volume->zdim;
  //////////////////////////////////////////////////
  ...
  transf_x_slice(dest,x,sy,sz, pixel_size); // use mid slice
  return create_slice(dest,sy,sz,pixel_size,pixel_size,dx);
}
                                        ...
                                        float pixel_size) const
{
  ...
  transf_y_slice(dest,y,sx,sz, pixel_size);  // use mid slice
  return create_slice(dest,sx,sz,pixel_size,pixel_size,dy);
}
