...
char*
_is_interfile(const char* fname)
{
	int c;
	FILE *fp;
	char *p = magicNumber;
...
is_interfile(const char* fname)
{
  ...
  if ( (img_fname=strrchr(fname,DIR_SEPARATOR)) == NULL) img_fname = fname;
  else img_fname++;
  hdr_fname = malloc(strlen(fname)+5); /* +5 : strlen(.hdr)+1 */
...
int 
free_interfile_header(char **ifh)
{
	int i=0;
	for (i=0; i<END_OF_KEYS; i++)
		if (ifh[i] != NULL) free(ifh[i]);
	free(ifh);
	return 1;
}

int	
get_block_singles(MatrixFile *mptr, float **pextended_uncor_singles, 
...
int 
interfile_open(MatrixFile *mptr)
{
  InterfileItem* item;
  FILE *fp;		
  Main_header *mh;
  ...
  if (mptr->interfile_header[NUM_Z_ELEMENTS] != NULL) {
    switch(elem_size) {
    case 1 : mptr->mhptr->file_type = Byte3dSinogram; break;
    case 2 : mptr->mhptr->file_type = Short3dSinogram; break;
    case 4 : mptr->mhptr->file_type = Float3dSinogram; break;
    }
  } else mh->file_type = PetVolume;
...
interfile_read(MatrixFile *mptr, int matnum, MatrixData *data, int dtype) 
{
  ...
  case Float3dSinogram:
	...
	else if (ifh[IMAGE_RELATIVE_START_TIME] &&
		sscanf(ifh[IMAGE_RELATIVE_START_TIME], "%d",&sec) == 1)
	{
