/*----------------------------------------------
   ...
   By Sune Høgild Keller, Rigshospitalet, Copenhagen
   24-09-2008
...
int main(int argc,char **argv) {
	...
	int riters = cimg_option("-ri",3,"number of inner relaxation iterations");
          ...
          bool display = cimg_option("-display",0,"Display option, 1 for on, 0 for off");
...
*/
  ...
  u0 = u;
	...
	CImgDisplay main_disp(u0,"Restored image",3,3,0,1);
	CImg<unsigned char> p_im(400,200,1,3,0);
	CImgDisplay p_disp(p_im, "Progress",3,3,0,1);
	...
	//		Ba(x,y,z) = 1/sqrt(sqr(ua-uc) + 0.0625*sqr(ue-uw+uae-uaw) + 0.0625*sqr(un-us+uan-uas) + eps2);
		...
		for (int j = 1; j<=riters; j++){
			...
			if(display){
				u.display(main_disp);
				p_im.fill(0).draw_text(10,40,white,0,12,1,"Fixed point = %i",i);
				p_im.draw_text(10,54,white,0,12,1,"Relax = %i",j);
				p_im.draw_text(10,10,white,0,12,1,"Regularizing the file:");
				p_im.draw_text(10,22,white,0,12,1,file_in);
				p_im.display(p_disp);
			}
			...
			if (r < thres) {
				if(display){
					p_im.draw_text(10,70,red,0,12,1,"Conv. threshold reached %i", conv_count);				
					p_im.draw_text(10,82,red,0,12,1,"times");				
					p_im.display(p_disp);
				}
				conv_count++;
				break;}
	...
	if(display){
		p_im.draw_text(10,100,white,0,12,1,"Output saved as:");
		p_im.draw_text(10,112,white,0,12,1,file_out);
